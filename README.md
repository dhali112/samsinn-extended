...
**`allowed-tools` frontmatter** (Anthropic-Skills compatibility) — parsed and preserved on the loaded skill as `Skill.allowedToolNames`, surfaced in the skill detail endpoint. **Documentary metadata, never a runtime sandbox.** The field declares which tools the skill *expects to exist*; it does not gate tool access. Agent tool access is decided by (a) pack activation in the room and (b) `AIAgentConfig.tools`. The data is consumed by two diagnostics: a boot-time warning when declared tools aren't in the registry (catches packaging bugs), and a per-eval coherence warning when an active skill's declared tools aren't in the agent's LLM-facing surface (catches room-specific mismatches — surfaced via `/api/diagnostics/evals/recent`). Unknown names emit one dedup'd startup warning per skill. Inline array (`[a, b]`), block list (`- a\n- b`), and single-scalar forms are all accepted. Pack-namespaced resolution is not yet implemented — names resolve against the global registry.
...
src/
  ...
  integrations/leitbild/  — Leitbild bridge: discovery client, room mirror, agent tools
...
Samsinn can bridge to [Leitbild](https://leitbild.samsinn.app) — a map-based control-center research platform — so AI agents and humans share live operational context from a running simulation. Two paths, both built on the same underlying discovery + WS client (`src/integrations/leitbild/`):

**Room mirror (V1).** Bind a Samsinn room to a Leitbild Control Instance via `PUT /api/rooms/:name/leitbild-mirror`. The mirror service walks Leitbild's discovery manifest (`GET /.well-known/leitbild`), opens a WebSocket subscription, and posts each Leitbild domain event into the room as a `system`-typed chat message. Reset events arrive as explicit boundary messages so agents know historical events no longer reflect current state. See `src/integrations/leitbild/mirror-service.ts`.

**Per-agent bindings (V2).** An AI agent's config can carry a `leitbildBinding: { baseUrl, instanceId, role: 'observer' | 'operator' }`. Bound agents gain four read tools (`lb_state`, `lb_object`, `lb_query`, `lb_scenario`) plus, in operator role, the generic `lb_command(kind, targets, payload)` write tool. The agent's context-builder filters mirror messages out of the LLM context (the agent has direct tool access) but preserves reset boundary notices. Commands carry stable `actorId` + `clientId` derived from the agent's name (`actor:samsinn:<slug>`, `client:samsinn:<slug>`) so Leitbild logs and any future auth gate cleanly map back to the issuing Samsinn agent.
...
| Version | Changes |
|---|---|
| v0.14.0 | **Leitbild integration — room mirror + per-agent bindings + command tool.** Samsinn now bridges to [Leitbild](https://leitbild.samsinn.app) (map-based control-center simulation platform). Three threads land together. **V1 room mirror** (`src/integrations/leitbild/mirror-service.ts`): bind a Samsinn room to a Leitbild Control Instance via `PUT /api/rooms/:name/leitbild-mirror`; live domain events arrive as `type: 'system'` chat messages stamped with `cause: { kind: 'external-mirror', name: '<baseUrl>:<instanceId>:<seq>[:event:<id>\|:reset-boundary\|:banner]' }`. Race-safe attach (open WS → buffer events → fetch snapshot → anchor at snapshot.seq → deliver events with seq > snapshot.seq). WS reconnect with exponential backoff + durable replay via `/events?afterSeq=N` + dedup by seq. Reset detection via `controlInstance.reset` event OR seq regression — both trigger snapshot refetch + boundary message + re-anchor (so bound agents know historical context was invalidated). Walks the deployment's `/.well-known/leitbild` discovery manifest — never hardcodes paths; validates required link rels on first fetch, fails loud if missing. **V2.A per-agent bindings**: `AIAgentConfig.leitbildBinding = { baseUrl, instanceId, role: 'observer' \| 'operator' }` unlocks `lb_state`, `lb_object(id)`, `lb_query(packId, kind, payload)`, `lb_scenario` read tools; per-(agent, instance) snapshot cache (5s TTL). Bound agent's context-builder drops `external-mirror` messages from LLM context (the agent has direct tool access) EXCEPT reset boundaries. **V2.B command tool**: operator-role agents gain `lb_command(kind, targets, payload)`; stable per-agent identity (`actor:samsinn:<slug>` + `client:samsinn:<slug>`) for journal provenance. `lb_reset` and `lb_clock_set` intentionally NOT exposed (reset/clock changes invalidate reasoning context — human-only). **Coordinated Leitbild-side work**: shipped discovery manifest (`/.well-known/leitbild` with HAL-style links + actions + capabilities + cors + auth posture), explicit `controlInstance.reset` durable event + monotonic seq across reset, per-CI capability manifest (`GET /api/control-instances/{id}/capabilities`), X-Forwarded-Proto handling. **Snapshot v24 → v25** (additive optional `RoomSnapshot.leitbildMirror`). **MessageCauseKind extended** with `'external-mirror'`. **+12 tests** (1471 → 1483). Auth deliberately deferred to the immediately-following phase per the colleague-share-link stage we're in. |
| v0.13.0 | **Pack-gated UI extensions (Path C) + biometrics + tool-surface manager.** Three threads land together. **Path C extension layer**: `PackManifest.ui_extensions: ["name"]` declares browser UI a pack expects; core's `KNOWN_UI_EXTENSIONS` map mounts the matching module on install, unmounts on uninstall. Lazy `import()` inside `mount()` so unused extension code stays out of the bundle path until activated. `postRenderProcessors` becomes a registry (`src/ui/modules/extensions/post-render-registry.ts`); mermaid + map self-register at module-load. `MessageCause.kind` extended with `'biometric'`; UI duplicate type collapsed to import from `messaging.ts`. Settings nav dynamically appends one row per registered extension panel. **Biometrics subsystem**: extractable `src/biometrics/` package wrapping MediaPipe Tasks Vision FaceLandmarker — runs entirely client-side (CDN load + GPU inference), emits live signals (attention, expression, blink rate, head pose). Inline widget in the message stream (mermaid/map pattern): consent → active → stopped state machine with multi-tab claim race (first-tab-wins via `biometric_capture_claimed` WS broadcast), MediaStream owned by the widget, three independent teardown paths (user stop / DOM unmount / page unload). Capture registry in-memory per House; agent-side `biometrics_start/_stop/_read` tools registered with `source.pack: 'biometrics'` so per-room activation gates visibility. Snapshot redactor at save time scrubs ephemeral content (`cause.kind === 'biometric'` → placeholder); cause + captureId preserved for causality. Configurable resolution (320×240 / 480×360 / 640×480). **`samsinn-packs/biometrics` pack** ships pack.json (declares the extension) + agent-facing skill with worked examples and ethics. Demo card "Biometric awareness" in the synthetic demos pack walks the user through install → consent → live capture → stop. **Tool Surface Manager** (`src/tool-surface/`): family compression collapses related tools into synthetic dispatchers (`fs` for 14 filesystem MCP tools, `geo_tools`, `pack_admin`, `codegen_tools`) — reduces per-request tool tokens 50%+ on the affected families. Provider-aware: Gemini falls back to flat list (strict tool-schema). Budget cap (2K-token default) prevents pathological surfaces. Addresses OpenAI tier-1 TPM saturation observed in prod. **+91 tests** (1141 → 1232 unit + integration). **Snapshot unchanged at v22** (all new fields additive optional). |
| v0.12.0 | **Automation extension — triggers, scenarios, scripts unified into a workflow platform.** Five focused phases turn the per-feature primitives into a composable system. **Triggers** (`Trigger.mode`) gain `'start-script'` and `'start-scenario'` modes — a trigger can now schedule a script in a room or kick off an instance-wide scenario; per-mode busy-check skips dispatch (without marking `lastFiredAt`) so triggers re-evaluate next tick when the target frees up. **Scenarios** gain three new ops: `inline-script` embeds a script literal directly in scenario YAML (parses via `parseScriptMd` → `scriptRunner.startWith(parsedScript)` → registers cleanup so abort-mid-wait propagates to the launched script); `branch-on-llm-decision` lets an LLM pick among DECLARED branches (op-handler whitelist remains the security boundary — the LLM chooses *order*, not *behavior*); ops gain optional named `id` labels so jumps survive op-list reordering. The scenario runner main loop refactored from `for (let i = ...)` to a while-driven `state.currentOpIndex` so branching ops can mutate the index in place; per-op visit counter (3-visit threshold) replaces a magic "max jumps" cap. **`write_scenario`** built-in tool mirrors `write_script` / `write_skill` / `write_tool` — agents can author scenarios at runtime; `ScenarioStore.upsert` writes to `$SAMSINN_HOME/packs/local/scenarios/`. **`MessageCause` field** on Message lets every persistence + transport surface (snapshot, room-export, WS broadcast, MCP get_messages) carry "this message exists because script X step N / scenario Y op M / trigger Z fired"; agent context-builder strips it from prompts so it stays metadata, not LLM noise. New LLM source tag `'scenario-branch'`. Documented prompt-injection limitation (`branch-on-llm-decision` over user-controlled context is suitable for friendly flows, not adversarially robust). **+25 tests** (validator, scheduler, store, parser, ops, message round-trip). 1155 → 1180 unit tests pass. **Snapshot unchanged at v22** (all new fields additive optional). |
| v0.11.0 | **Codebase-health system + audit-driven cleanup.** New `bun run health` runs `tsc`, `type-coverage`, escape-hatch grep, `dependency-cruiser`, and `knip`, writing `.health/YYYY-MM-DD.md` (delta vs `.health/baseline.md`). Pre-push git hook (~5s) validates typecheck + dependency boundaries, blocks on UI→core/MCP→UI violations, bypassable with `--no-verify`. Install via `bash scripts/install-hooks.sh` (worktree-safe via `core.hooksPath`). Two project-local skills (`.claude/skills/health-audit`, `.claude/skills/refactor-guarded`) plus a `/health` slash command — restart Claude Code to register after pulling. **Audit findings acted on:** dropped the `scriptHook` `lateBinding` (22 → 21 slots) — replaced with a direct `onScriptMessage` callback in `RoomCallbacks` so the dispatch site is visible at `room.post()` instead of behind a proxy; `zod` now declared explicitly in `package.json` (was transitive via MCP SDK); 9 dead exports removed (incl. `prompt-model-editors.ts` superseded by `prompt-toggles/`); `parseRetryAfter` consolidated into one source-of-truth in `llm/errors.ts`; UI countdown formatting extracted to `ui/lib/format-retry.ts`; `app.ts` shrunk by extracting `ui-bootstrap-footer.ts` (theme + version + repo link). **+12 tests** for `/api/agents` and `/api/agents/.../triggers` negative paths. Documented architecture decisions in CLAUDE.md "Rejected refactors" section preserved and referenced in skill triggers. **Snapshot version unchanged** (v22). |
