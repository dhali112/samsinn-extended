...
describe('createTriggerScheduler', () => {
  ...
  test('start-script trigger calls startScript when target room is free', async () => {
    const trigger: Trigger = {
      id: 't1', name: 'kick off', prompt: '', mode: 'start-script',
      intervalSec: 60, enabled: true, roomId: 'r1', targetName: 'demo',
      lastFiredAt: 0,
    }
    const agent = mkAgent('A', 'human', [trigger])
    const room = mkRoom('r1', agent)
    const calls: Array<{ roomId: string; name: string }> = []
    let busy = false
    const sched = createTriggerScheduler({
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      team: { listAgents: () => [agent as any], getAgent: () => agent as any } as any,
    ...
    await sched.tickNow()
    expect(calls).toEqual([{ roomId: 'r1', name: 'demo' }])
    expect(agent.triggers[0]!.lastFiredAt).toBe(1_000_000)
    sched.stop()
  })

  test('start-script trigger is suppressed while target room already running a script', async () => {
    const trigger: Trigger = {
      id: 't1', name: 'kick off', prompt: '', mode: 'start-script',
      intervalSec: 60, enabled: true, roomId: 'r1', targetName: 'demo',
      lastFiredAt: 0,
    ...
    // Crucially: lastFiredAt unchanged so the trigger re-evaluates next tick.
